...
class AcrCommandsTests(ScenarioTest):
    ...
    def test_acr_with_dual_stack_endpoints(self, resource_group, resource_group_location):
        ...
        self.cmd('acr create --name {registry_name} --resource-group {rg} --sku premium -l eastus',
                 checks=[self.check('dualStackEndpointsEnabled', False)])
        self.cmd('acr update --name {registry_name} --resource-group {rg} --dual-stack-endpoints-enabled true',
                 checks=[self.check('dualStackEndpointsEnabled', True)])
        self.cmd('acr update --name {registry_name} --resource-group {rg} --dual-stack-endpoints-enabled false',
                 checks=[self.check('dualStackEndpointsEnabled', False)])
